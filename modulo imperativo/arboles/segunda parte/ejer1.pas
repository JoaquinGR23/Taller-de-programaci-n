...
{ Retornar la cantidad de códigos que existen en el árbol que son menores que un valor que se recibe como parámetro }
  ...
  begin
    if(a<>nil)then begin 
      if(a^.dato.codigo>=cod)then CantidadDeCodigosMenores:=CantidadDeCodigosMenores(a^.HI,cod)
      else CantidadDeCodigosMenores:=CantidadDeCodigosMenores(a^.HI,cod)+CantidadDeCodigosMenores(a^.HD,cod)+1;
    end
    else CantidadDeCodigosMenores:=0;
  end;
...
comprendidos entre los dos valores recibidos (sin incluir). }
  ...
  begin
    if(a<>nil)then begin 
      if((a^.dato.codigo>codigo1)and(a^.dato.codigo<codigo2)) then ObtenerMontoTotalEntreDosCodigos:=ObtenerMontoTotalEntreDosCodigos(a^.HI,codigo1,codigo2)
      +ObtenerMontoTotalEntreDosCodigos(a^.HD,codigo1,codigo2)+a^.dato.montoTotal
      else if(a^.dato.codigo>codigo1)then ObtenerMontoTotalEntreDosCodigos:=ObtenerMontoTotalEntreDosCodigos(a^.HI,codigo1,codigo2)
...
procedure ModuloF (a: arbol);
{retornar cantidad de codigos donde su cantidad total de productos superan al codigo ingresado por teclado entre 2 codigos ingresados por teclado}
  
  function cantTotalEntreDosCodigos (a: arbol; codigo1, codigo2,valor: integer): integer;
  begin
    if(a<>nil)then begin 
      if((a^.dato.codigo>codigo1)and(a^.dato.codigo<codigo2)) then begin 
        if(a^.dato.cantTotalUnidades>valor) then cantTotalEntreDosCodigos:=cantTotalEntreDosCodigos(a^.HI,codigo1,codigo2,valor)+cantTotalEntreDosCodigos(a^.HD,codigo1,codigo2,valor)+1 
      end
      {else if(a^.dato.codigo>codigo1)then cantTotalEntreDosCodigos:=cantTotalEntreDosCodigos(a^.HI,codigo1,codigo2,valor)
      else cantTotalEntreDosCodigos:=cantTotalEntreDosCodigos(a^.HD,codigo1,codigo2,valor)}
    end
    else cantTotalEntreDosCodigos:=0;
  end;
   
var codigo1, codigo2, valor,cant: integer;
begin
  writeln;
  writeln ('----- Modulo F ----->');
  writeln;
  ...
  readln (valor);
  cant:= cantTotalEntreDosCodigos (a, codigo1, codigo2,valor);
  if (cant = 0) 
  then writeln ('No hay codigos entre ', codigo1, ' y ', codigo2)
  else begin
         writeln;
         writeln ('La cantidad de codigos entre el codigo', codigo1, ' y el codigo : ', codigo2, ' que superan al valor ingresado es: ', cant); 
         writeln;
       end;
  writeln;
  writeln ('-----------------------------------------------');
